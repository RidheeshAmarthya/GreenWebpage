...
const QUALITY_SAMPLE_PRINT_STYLES = `
    body { font-family: 'Inter', system-ui, -apple-system, sans-serif; margin: 0; padding: 0; background: #525659; }
    ...
    @media print {
        ...
        .spec-card {
            box-shadow: none;
            width: 210mm;
            /* Slightly under A4 — Safari rounds 297mm up and adds a blank page */
            height: 296.5mm;
            max-height: 296.5mm;
            overflow: hidden;
            margin: 0;
        ...
        /* Start each following card on a new page — never break-after on the card itself */
        .spec-card + .spec-card {
            page-break-before: always !important;
            break-before: page !important;
        }

        .page-container > .spec-card:last-of-type {
            margin-bottom: -2px;
            page-break-after: avoid !important;
            break-after: avoid-page !important;
        }
        ...
        html.single-page,
        html.single-page body {
            height: auto !important;
            overflow: hidden;
        }

        html.single-page .page-container {
            overflow: hidden;
        }
...
function renderQualitySampleCard(item, labelImageUrl, imgUrl) {
    return `
        <div class="spec-card">
            <div class="top-info">
...
function getStockPrintBaseUrl() {
    const href = window.location.href.split('#')[0].split('?')[0];
    return href.substring(0, href.lastIndexOf('/') + 1);
}

/** Must run synchronously from the user click — Safari blocks window.open after await. */
function openQualitySamplePrintWindow() {
...
function writeQualitySamplePrintLoading(printWindow, message) {
    ...
    printWindow.document.write(`
        <html>
        <head>
            <base href="${getStockPrintBaseUrl()}">
...
function countQualitySampleCards(cardsHtml) {
    return (cardsHtml.match(/class="spec-card"/g) || []).length;
}

function writeQualitySamplePrintDocument(printWindow, title, cardsHtml) {
    if (!printWindow || printWindow.closed) return;
    const cardCount = countQualitySampleCards(cardsHtml);
    const htmlClass = cardCount === 1 ? 'single-page' : 'multi-page';
    printWindow.document.open();
    printWindow.document.write(`
        ...
        <head>
            ...
            <style>${QUALITY_SAMPLE_PRINT_STYLES}</style>
        </head>
        <body>
            <div class="page-container">${cardsHtml}</div>
        </body>
        </html>
    `);
    printWindow.document.close();
...
function waitForQualitySamplePrintReady(printWindow) {
    return new Promise((resolve) => {
        ...
        const imageReady = (img) => new Promise((done) => {
            if (img.complete) {
                done();
                return;
            }
...
async function triggerQualitySamplePrint(printWindow) {
    await waitForQualitySamplePrintReady(printWindow);
    if (!printWindow || printWindow.closed) return;
    printWindow.focus();
    printWindow.print();
}
...
async function generateStockPDF(id) {
    ...
    const printWindow = openQualitySamplePrintWindow();
    if (!printWindow) return;

    writeQualitySamplePrintLoading(printWindow, `Preparing ${item.article_no || 'sample'}…`);
    ...
    try {
        ...
        writeQualitySamplePrintDocument(
            printWindow,
            `Spec Sheet - ${item.article_no}`,
            renderQualitySampleCard(item, labelImageUrl, imgUrl)
        );
        await triggerQualitySamplePrint(printWindow);
    } catch (err) {
...
async function generateBatchStockPDF() {
    ...
    writeQualitySamplePrintLoading(printWindow, 'Generating batch report…');
    ...
    try {
        ...
        for (let i = 0; i < selectedItems.length; i++) {
            ...
            const progressLabel = `Rendering ${i + 1} of ${selectedItems.length}…`;
            writeQualitySamplePrintLoading(printWindow, progressLabel);
            if (loadingOverlay) {
        ...
        const cardsHtml = selectedItems.map(item => renderQualitySampleCard(
            item,
            item.temp_rendered_label || '',
            item.resolved_url || placeholderImg
        )).join('');

        writeQualitySamplePrintDocument(printWindow, 'Green Batch Spec Report', cardsHtml);
        await triggerQualitySamplePrint(printWindow);
        clearStockSelection();
    } catch (err) {
        if (!printWindow.closed) printWindow.close();
